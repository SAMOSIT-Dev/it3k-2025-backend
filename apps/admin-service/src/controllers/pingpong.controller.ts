...
export const createPingpongMatch = async (req: Request, res: Response) => {
    try {
        ...
        const query = `INSERT INTO pingpong_matches (type, team_A_id, team_B_id, time, locationId, team_A_number, team_B_number) VALUES (?, ?, ?, ?, ?, ?, ?)`;
...
export const updatePingpongMatch = async (req: Request, res: Response) => {
    try {
        ...
        const query = `UPDATE pingpong_matches SET type = ?, team_A_id = ?, team_B_id = ?, time = ?, locationId = ?, team_A_number = ?, team_B_number = ? WHERE id = ?`;
